...
function setup_ridge_plot(data, container, all) {
  ...
  if(all)
    ridge_height = d3.mean(data.map((d)=>d3.max(d.all.slice(1,-1))*0.75))
  ...
  if(all)
    ridge_plot.distros.append("path")
              .classed("line", true)
              .attr("d", (d)=>ridge_plot.draw.outline(d.all.slice(1,-1)))
